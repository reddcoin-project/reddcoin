...
class AsmapTest(BitcoinTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test reddcoind with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_asmap_with_absolute_path(self):
        self.log.info('Test reddcoind -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test reddcoind -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_default_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info('Test reddcoind {} (using default map file)'.format(arg))
            self.stop_node(0)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test reddcoind -asmap with missing default map file')
        self.stop_node(0)
        msg = "Error: Could not find asmap file \"{}\"".format(self.default_asmap)
        self.node.assert_start_raises_init_error(extra_args=['-asmap'], expected_msg=msg)

    def test_empty_asmap(self):
        self.log.info('Test reddcoind -asmap with empty map file')
        self.stop_node(0)
