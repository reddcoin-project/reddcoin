...
class WalletDumpTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        # ReddCoin: importwallet rescan over the PoS chain can exceed 60s (=120/2)
        # under CPU contention from concurrent staking tests. Raise for headroom.
    ...
    def run_test(self):
        ...
        dump_wallet = self.nodes[0].get_wallet_rpc("dump")
        ...
        for address_type in ['legacy', 'p2sh-segwit', 'bech32']:
            for _ in range(test_addr_count):
                addr = dump_wallet.getnewaddress(address_type=address_type)
                vaddr = dump_wallet.getaddressinfo(addr)  # required to get hd keypath
                addrs.append(vaddr)

        # Test scripts dump by adding a 1-of-1 multisig address
        multisig_addr = dump_wallet.addmultisigaddress(1, [addrs[1]["address"]])["address"]
        ...
        dump_time = block_time + 1
        self.nodes[0].setmocktime(dump_time)
        self.nodes[0].mocktime = dump_time
        ...
        result = dump_wallet.dumpwallet(wallet_unenc_dump)
        ...
        # encrypt wallet, unlock and dump
        dump_wallet.encryptwallet('test')
        dump_wallet.walletpassphrase('test', 100)
        # Should be a no-op:
        dump_wallet.keypoolrefill()
        dump_wallet.dumpwallet(wallet_enc_dump)
        ...
        assert_raises_rpc_error(-8, "already exists", lambda: dump_wallet.dumpwallet(wallet_enc_dump))
        ...
        result = w2.getaddressinfo(multisig_addr)
