...
class MultiWalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        w5 = wallet("w5")
        w5.generatetoaddress(nblocks=1, address=w5.getnewaddress())
        ...
        if self.options.descriptors:
            exp_stderr = r"Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another reddcoind?"
        else:
        ...
        wallets[0].generatetoaddress(nblocks=1, address=wallets[0].getnewaddress())
        ...
        w1.generatetoaddress(nblocks=1, address=w1.getnewaddress())
        ...
        if self.options.descriptors:
            assert_raises_rpc_error(-4, "Wallet file verification failed. SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another reddcoind?", self.nodes[0].loadwallet, wallet_names[0])
        else:
