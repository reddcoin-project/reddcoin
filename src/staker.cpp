...
void CStakeman::ThreadStaker(std::shared_ptr<interfaces::StakingWallet> staking_wallet, ChainstateManager* chainman, CConnman* connman, CTxMemPool* mempool, std::thread::id thread_id, std::atomic<bool> &running, CThreadInterrupt& interrupt)
{
    ...
    // Widen the startup window on demand, so a test can land an unloadwallet
    // inside it rather than hoping to. Debug-only and zero by default; the
    // window this sits in is the one the isUnloading() check below closes.
    ...
    // the unload until the handler above is connected, several statements into
    // the new thread. An unloadwallet arriving in that window fires
    // NotifyUnload with no subscriber, so the handler above would wait forever
    // for a notification that has already been sent, while this thread holds
    // the reference UnloadWallet() is blocked on. The wallet is marked before
    // the notification goes out, so asking now covers the whole window.
    if (staking_wallet->isUnloading()) {
