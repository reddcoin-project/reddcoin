...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    std::vector<std::string> hidden_args = {
        ...
        "-checkupdates", "-choosedatadir", "-lang=<lang>", "-min", "-resetguisettings", "-splash", "-uiplatform"};
    ...
    argsman.AddArg("-stakerstartdelay=<n>", "Pause each staking thread for <n> milliseconds before it subscribes to its wallet's unload notification, widening the startup window so a test can unload the wallet inside it (default: 0)", ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::DEBUG_TEST);
